...
#     users_ref.document(keystore.get('address')).set({'keystore':keystore,'score':0})
...
class Blockchain:
    ...
    def create_block(self, proof, previous_hash,crimeType,description,mediaURL,urgency,crimeTime,score,uid,address,dateOfIncident,userScore):
        block = {'index': len(self.chain) + 1,
                 ...
                 'userScore': userScore,
                 'previous_hash': previous_hash,'isAlert':0,'view':1,'useful':0,'fake':0,'suspectScore':0,'mentalScore':0,'mediaScore':0,'likelyhood':0,"parentTip":len(self.chain) + 1,}
